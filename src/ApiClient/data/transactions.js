...
export const EditSearchTerm = (e) => {
  this.setState({ searchTerm: e.Target.value });
};

export const dynamicSearch = () => {
  return this.transactions.amount.filter((amt) =>
    amt.includes(this.transactions.searchTerm)
  );
};

export class Amt extends Component {
  render() {
    return <div>{this.props.amt}</div>;
  }
}

export default AmtContainer;
